...
app.post("/new-driver", async (req, res) => {
  try {
    ...
    return res.render("new-driver", {
      data: {
        err: true,
        message: "Failed to create new driver. Try again later.",
      },
    });
  }
});
...
app.get("/driver/delete/:id", async (req, res) => {
  try {
    const driver = await ClientModel.findById(req.params.id);
    return res.render("delete-driver", {
      driver,
      found: driver != null ? true : false,
    });
  } catch (e) {
...
const uri = "mongodb+srv://glenn:_Matsiwe97@cluster0.r9zki.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
const clientOptions = { serverApi: { version: '1', strict: true, deprecationErrors: true } };
async function run() {
  try {
    // Create a Mongoose client with a MongoClientOptions object to set the Stable API version
    await mongoose.connect(uri, clientOptions);
    let doc = await mongoose.connection.db.admin().command({ ping: 1 });
    console.log("Pinged your deployment. You successfully connected to MongoDB!", JSON.stringify(doc));
