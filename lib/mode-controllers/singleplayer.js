...
module.exports = async (req, res) => {
  ...
  if (req.user.gameSessId) {
    ...
    const responseMsg = await game.handleUserResponse(userMsg, req);
  ...
  const command = commands.singlePlayerCommands(req).find(c => c.code === userMsg);
  ...
  // const isGameNumValid = gameNum >= 0 && gameNum < GameFactory.getGames().length;
