...
class Tingurides {
    ...
    async handleUserResponse(answer) {
        ...
        if (answer.trim().toLowerCase() == 'logou') {
            this.reset()
            // this.state = 'gameover'
            this.qnState = ''
        ...
        if (this.data?.user?.loggedIn == true && this.data?.user?.user?.type == 'client') {
            let action = await this.client.handleUserInput(answer)
            messages.push(action)
            return messages
        } else if (this.data?.user?.loggedIn == true && this.data?.user?.user?.type == 'driver') {
            ...
            if (isNumeric(answer)) {
                ...
                switch(this.qnState) {
                    ...
                    case 2:
                        this.qnState = ''
                        messages.push('For further assistance please contact Customer Care on *0719445433*. \n\n' + this.gameMessage)
                        return messages
                    default:
